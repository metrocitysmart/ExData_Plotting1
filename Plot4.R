...
# Adding a column of day names of datetime column
hh_Pconsumption <- hh_Pconsumption %>%
  mutate(day = format(datetime, "%a"))

# Generating fourth plot and saving it
...
with(hh_Pconsumption, plot(datetime, Sub_metering_1,
                           type = "l", col = "black",
                           ylab = "Energy sub metering", xlab = NA,
                           xaxt = "n"))
with(hh_Pconsumption, lines(datetime, Sub_metering_2, type = "l", col = "red"))
with(hh_Pconsumption, lines(datetime, Sub_metering_3, type = "l", col = "blue"))
axis(1, at = c(hh_Pconsumption$datetime[1], 
               hh_Pconsumption$datetime[ceiling(nrow(hh_Pconsumption)/2)],
               hh_Pconsumption$datetime[nrow(hh_Pconsumption)]),
     labels = c(hh_Pconsumption$day[1], 
...
legend("topright", inset = 0.02,
       ...
       lty = 1, box.lty = 0, text.font = 1, cex = 0.8,
       col = c("black", "red", "blue"))

with(hh_Pconsumption, plot(datetime, Voltage, type = "l", xaxt = "n"))
axis(1, at = c(hh_Pconsumption$datetime[1], 
               hh_Pconsumption$datetime[ceiling(nrow(hh_Pconsumption)/2)],
               hh_Pconsumption$datetime[nrow(hh_Pconsumption)]),
     labels = c(hh_Pconsumption$day[1], 
...
with(hh_Pconsumption, plot(datetime, Global_reactive_power,
                           type = "l", xaxt = "n"))
axis(1, at = c(hh_Pconsumption$datetime[1], 
               hh_Pconsumption$datetime[ceiling(nrow(hh_Pconsumption)/2)],
               hh_Pconsumption$datetime[nrow(hh_Pconsumption)]),
